...
const getRoomData = async ({params} : {params: any})=> {
  const res = await fetch(
    `https://royalapp-backend.onrender.com/api/rooms/${params.id}?populate=*`, 
    {
      next: {
        revalidate: 0,
      },
...
const getReservationData = async ()=> {
  const res = await fetch(
    `https://royalapp-backend.onrender.com/api/reservations?populate=*`, 
    {
      next: {
        revalidate: 0,
      },
...
const RoomDetails = async ({params} : {params: any}) => {
  ...
  const imgURL = `https://royalapp-backend.onrender.com${room.data.attributes.image.data.attributes.url}`;
